...
include '../header_footer/user_header.php';
    ...
    $check = mysqli_query($con, "SELECT c.CategoryID, ProductID, ProductName, ThumbnailImage, 
...
<body id="pro-page">
    ...
    <div class="topnav" id="myTopnav">
        <a href="product_page.php">Tất cả</a>
    ...
    <div class="main">
                ...
                <div class="col1">
                    <div class="sort-box">
                        <select name="sort" id="sort" class="form-control" onchange="this.form.submit();">
                            ...
                            <option value="low"
                                <?php if (isset($_GET['sort']) && $_GET['sort'] == "low") {echo "selected";} ?>>Giá:
                                Thấp đến Cao</option>
                            <option value="high"
                                <?php if (isset($_GET['sort']) && $_GET['sort'] == "high") {echo "selected";} ?>>Giá:
                                Cao đến Thấp</option>
                        </select>
                    </div>
                </div>
        ...
        <div class="container-p" id="1">
            <?php
                ...
                $sort = "";
                $sort_option = "";
                $search = "WHERE ProductName LIKE '%$keyword%'";
    ...
    // check if user use a different port
    $find_port = mysqli_query($con, "SHOW VARIABLES WHERE Variable_name = 'port'");
    $port = mysqli_fetch_array($find_port);
    $value =  $port['Value'];
    // PAGINATION
    if ($value != 3306) {
        try {
            $pdo = new PDO(
                "mysql:host=$sname:$value;dbname=$db_name",
                $uname,
                $password,
    ...
    $all_products = mysqli_query(
        $con, "SELECT c.CategoryID, ProductID, ProductName, ThumbnailImage, 
                    min(Price) AS `min`, max(Price) AS `max`, SUM(ProductQuantity) AS Quantity 
                    FROM `Categories` c INNER JOIN product p ON c.CategoryID = p.CategoryID $search                       
    ...
    if (isset($_GET['sort'])) {
        if ($_GET['sort'] == "new") { //phân loại theo ngày thêm sp và thứ tự giảm dần
            for ($i = 1; $i <= $page; $i++) {
                printf(
                    "<a %shref='search.php?sort=new&page=%s'>%s</a> ",
            ...
            for ($i = 1; $i <= $page; $i++) {
                printf(
                    "<a %shref='search.php?sort=low&page=%s'>%s</a> ",
                    $i == $pageNow ? "class='current' " : "",
                    $i,
                    $i
                );
            ...
            for ($i = 1; $i <= $page; $i++) {
                printf(
                    "<a %shref='search.php?sort=high&page=%s'>%s</a> ",
                    $i == $pageNow ? "class='current' " : "",
                    $i,
                    $i
                );
