...
session_start ();
    ...
    if(strlen($phone) > 10 || strlen($phone) < 10 || !preg_match('/^[0-9]+$/', $phone)){
        header("Location: checkout.php?error=Số Điện Thoại Không Hợp Lệ");
        exit();
    } //check name
    elseif(strlen($fname) > 30){
        header("Location: checkout.php?error=Tên Không Được Quá 30 Ký Tự");
        exit();
    } //check address
    elseif(strlen($address) > 200){
        header("Location: checkout.php?error=Địa Chỉ Không Được Quá 200 Ký Tự");
        exit();
    } //check address
    elseif(strlen($note) > 5000){
        header("Location: checkout.php?error=Ghi Chú Quá Dài");
        exit();
    } else {
        //tổng đơn
        $total = $_POST['total'];
        ...
        $cart_update = mysqli_query($con, "DELETE FROM Anon_Cart_Product WHERE ProductID = '$productID'");
        }
