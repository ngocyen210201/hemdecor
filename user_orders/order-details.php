            ...
            <div class="row">
                    ...
                    <div class="order_details_iner">
                        ...
                        while ($row1 = mysqli_fetch_assoc($sql1)){ ?>
                                <tr>
                                    ...
                                    <td><span style="cursor: auto;"><?php echo number_format($row1["Price"]) ?></span>
                                    </td>
                                    <td><span style="cursor: auto;"><?php echo $row1["OrderQuantity"] ?></span></td>
                                    <td> <span
                                            style="cursor: auto;"><?php echo number_format($row1["QuantityPrice"]) ?></span>
                                    </td>
                                </tr>
                                <?php $i++;} ?>
                            ...
                            <tr>
                                <td colspan="4"><span style="cursor: auto;">Tổng tiền hàng: <br>Phí Vận Chuyển (Mặc
                                        định):</span> <br><b>Tổng
                                        Đơn</b></td>
                                <td> <span style="cursor: auto;"> <?php echo number_format($row["TotalQuPrice"]) ?>
                                        <br>30,000
                ...
                <div class="col-lg-5 col-lx-4">
                    <div class="single_confirmation_details">
                        ...
                        <p><b>Ghi Chú: </b><span><?php echo $row["Note"] ?></span></p>
                        <?php if($row["OrderStatus"] == "Chờ Xác Nhận"){ ?>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <h5>Chú ý: Nếu đơn hàng của bạn sử dụng phương thức thanh toán Banking,
                                vui lòng nhắn tin cho fanpage <a
                                    href="https://www.facebook.com/xuonggo.hemdecor">HemDecor</a>
                                theo cú pháp: Mã Đơn, Họ và Tên, SĐT, STK cùng chi nhánh ngân hàng để
                                được xác nhận và hoàn trả tiền sản phẩm.</h5>
                        </div>
                        <div class="modal-footer">
                            <a
                                href="order-details.php?oid=<?php echo $row["OrderID"] ?>&cancel=<?php echo $row["OrderID"] ?>">
                                <!-- data-toggle="modal" data-target="#demo-2"
                                    data-dismiss="modal" -->
                                <button type="button" class="btn btn-danger" data-toggle="modal"
                                    data-target="#demo-2">Hủy Đơn Hàng</button>
                            </a>
                            ...
                            if (isset($_GET['cancel'])){
                                ...
                                // tìm sản phẩm nằm trong đơn vừa hủy
                                $find_product = mysqli_query($con, "SELECT * FROM Order_Product WHERE OrderID = '$_GET[cancel]'");
                                while ($update = mysqli_fetch_array($find_product)) {
                                    // update lại số lượng sp trong đơn vừa hủy
                                    $quantity_update = $update['OrderQuantity'];
                                    $productID = $update['ProductID'];
                                    mysqli_query($con, "UPDATE Product SET ProductQuantity = ProductQuantity+'$quantity_update' WHERE ProductID = '$productID'");
                                }
                                if ($cancel) {
                                    // show demo-2 in 3s
