...
if ($_GET['action'] == 'edit') {
    ...
    if ($num == 1) {
        ...
        exit();
    }elseif (strlen($proid) > 10) {
        echo "<script> alert('Mã sản phẩm không được vượt quá 10 ký tự!!');
        window.location.href = 'edit_product.php?cid=$cid';</script>";
        exit();
    }elseif (strlen($size) > 10) {
        echo "<script> alert('Size không hợp lệ!!');
        window.location.href = 'edit_product.php?cid=$cid';</script>";
        exit();
    }elseif (strlen($price) > 10 || $price < 0 || $price > 4294967295) {
        echo "<script> alert('Giá không hợp lệ!!');
        window.location.href = 'edit_product.php?cid=$cid';</script>";
        exit();
    }elseif (strlen($quantity) > 10 || $quantity < 0 || $quantity > 4294967295) {
